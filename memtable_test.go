...
func TestMemTable_put(t *testing.T) {
	...
	if mt.data.Size() != length {
		t.Errorf("put, expected data size=%d, actual data size=%d", length, mt.data.Size())
	}
	expectedSize := (keySize + valueSize) * length
	if mt.bytes() != expectedSize {
		t.Errorf("put, expected mt size=%d, actual mt size=%d", expectedSize, mt.bytes())
	}
}
...
func TestMemTable_delete(t *testing.T) {
	...
	if mt.data.Size() != length {
		t.Errorf("delete, expected data size=%d, actual data size=%d", length, mt.data.Size())
	}
	expectedSize := keySize * length
	if mt.bytes() != expectedSize {
		t.Errorf("delete, expected mt size=%d, actual mt size=%d", expectedSize, mt.bytes())
	}
}
...
func TestMemTable_clear(t *testing.T) {
	...
	mt.clear()
	if mt.data.Size() != 0 {
		t.Errorf("clear, expected data size=0, actual=%d", mt.data.Size())
	}
	if mt.bytes() != 0 {
		t.Errorf("clear, expected mt size=0, actual=%d", mt.data.Size())
	}
}
